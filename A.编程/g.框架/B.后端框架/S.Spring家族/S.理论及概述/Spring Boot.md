...
public class Person {
   ...
   //lastName必须是邮箱格式
    //@Email 是与 @Validated 整合使用的，表示 lastName 需要符合邮箱的校验规则
    @Email
...
–file:./config/                                 //此处的file 指定是项目的总目录

–file:./

–classpath:/config/                         //此处的classpath      指的是resources  目录下
...
​		(访问当前项目的任何路径，只要发送静态资源请求，且没有人响应，就会去以下目录寻找)
...
#ids : methods for dealing with id attributes that might be repeated (for example, as a result of an iteration).
   ...
   <div th:object="${session.user}">
    ...
    Link URL Expressions: @{...}：定义URL；
    		@{/order/process(execId=${execId},execType='FAST')}     //携带多个参数
